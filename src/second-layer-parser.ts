...
const setCollectData: Function = async (htmlCodeList: string[]): Promise<SecondLayerDataType[]> => {
  const result: SecondLayerDataType[] = await Promise.all(
    htmlCodeList.map((value: string): SecondLayerDataType => ({
...
const splitHtmlCode: Function = (htmlCode: string): string[] | null => htmlCode.match(/<tr class="briefCitRow">[\w\W]*?<!--this is customized <screens\/briefcit_cht\.html>-->/gi);

const collectTargetHtmlCode: Function = (htmlCode: string): string | null => {
  const result: string[] | null = htmlCode.match(/<table [\w\W]*? cellpadding="3"[\w\W]*?>[\w\W]*?<!-- END BROWSELIST\/BRIEFCIT AREA -->/gi);
...
export const secondLayerParser: Function = async (htmlCode: string): Promise<SecondLayerDataType[]> => {
  // To aim target data
